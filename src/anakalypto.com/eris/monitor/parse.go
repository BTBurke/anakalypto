...
func createFlagSet() *pflag.FlagSet {
	pf := pflag.NewFlagSet("eris", pflag.ContinueOnError)
	pf.Usage = func() {
		fmt.Printf("Usage of eris:\neris -i <identifier> <options> mycommand\neris -i <identifier> <options> -- mycommand <mycommand-options>\n")
		fmt.Printf("\n%s", pf.FlagUsagesWrapped(10))
		fmt.Printf("\n\nFor unknown flag errors, add an empty flag separator (--) between the flags for eris and your command.  Example:\n\neris -i id -c config.yml -- mycommand --otherflag\n")
	}
